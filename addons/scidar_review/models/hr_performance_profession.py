...
class HrPerformanceComms(models.Model): 
    _name = "hr.performance.profession" 
    _description = "Performance Competency Professionalism"

    res_model = fields.Char(string="Related Model", default="hr.performance", readonly=True)
    objectivesprof = fields.Char(
    ...
    descriptionsprof = fields.Selection([
    ...
    employee_ratingprof = fields.Selection([
        ('1', '1: Unsatisfactory'),
    ...
    reviewer_ratingprof = fields.Selection([
        ('1', '1: Unsatisfactory'),
        ('2', '2: Needs Improvement'),
        ('3', '3: Meets Expectation'),
        ('4', '4: Exceeds Expectation'),
    ...
    commentsprof = fields.Html(string="Rating Comments")


    @api.onchange('descriptionsprof')
    def _onchange_description(self):
        ...
        if self.descriptionsprof:
            self.objectivesprof = competency_objectives_map.get(self.descriptionsprof, '')
        else:
            self.objectivesprof = ''